...
@testset "registry + traits" begin
    rels = all_relations()
    @test length(rels) == 115        # universal-only: model-specific (spin glass, Drude mobility, single-band Hall) moved to QAtlas
    ...
    @test length(all_relations(; domain=:spectral)) == 12
    @test length(all_relations(; domain=:keldysh)) == 11
