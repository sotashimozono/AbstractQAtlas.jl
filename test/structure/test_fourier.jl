...
@testset "each quantity's representation" begin
    ...
    @test representation(DynamicalCorrelation()) == (RealSpace(), TimeDomain())
...
@testset "Fourier-conjugate quantity + pairing" begin
    ...
    @test fourier_conjugate_quantity(DynamicalCorrelation()) === DynamicalStructureFactor
    ...
    @test fourier_pair(DynamicalStructureFactor(), DynamicalCorrelation())
    ...
    rG = representation(DynamicalCorrelation())
