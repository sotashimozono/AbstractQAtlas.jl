...
function card_jsonl(io::IO, cards)
    for c in cards
        print(
            ...
            _json_str(c.route),
            ",\"mechanism\":",
            _json_str(c.mechanism),
            ",\"independence\":",
            _json_str(c.independence),
            ",\"status\":",
            _json_str(c.status),
            ",\"subject\":",
            _card_json_num(c.subject),
            ",\"error_bar\":",
            _card_json_num(c.error_bar),
            ",\"independent\":",
            _card_json_arr(c.independent),
            ",\"atol\":",
            _card_json_num(c.atol),
            ",\"refs\":",
            _card_json_strarr(c.refs),
            ",\"provenance\":",
            _json_str(c.provenance),
            "}\n",
        )
    end
    return nothing
